...
def visualize_features(data):
    ...
    fig, axes = plt.subplots(2,2,figsize=16, fontweight='bold')
    fig.suptitle('House Features vs Price', fontsize=16, fontweight='bold')
    # TODO: Plot 1 (top left): SquareFeet vs Price [0,0]
    ...
    # TODO: Plot 2 (top right): Bedrooms vs Price [0,1]
    #       - scatter plot, color='green', alpha=0.6
    #       - labels and title
    #       - grid
    axes[1, 0].scatter(data['Bedrooms'], data['Price'], color='red', alpha=0.6)
    axes[1, 0].set_xlabel('Bedrooms (Number of Bedrooms)')
    axes[1, 0].set_ylabel('Price ($)')
    axes[1, 0].set_title('Bedrooms vs Price')
    axes[1, 0].grid(True, alpha=0.3)
    # TODO: Plot 3 (bottom left): Bathrooms vs Price [1,0]
    #       - scatter plot, color='red', alpha=0.6
    #       - labels and title
    #       - grid
    axes[1, 0].scatter(data['Bathrooms'], data['Price'], color='purple', alpha=0.6)
    axes[1, 0].set_xlabel('Bathrooms (Number of Bathrooms)')
    axes[1, 0].set_ylabel('Price ($)')
    axes[1, 0].set_title('Bathrooms vs Price')
    axes[1, 0].grid(True, alpha=0.3)
    # TODO: Plot 4 (bottom right): Age vs Price [1,1]
...
def split_data(X, y):
    ...
    print(f"\NOTE: We're NOT scaling features here so coefficients are easy to interpret!")
...
def make_prediction(model, sqft, bedrooms, bathrooms, age):
    ...
    house_features = pd.DataFrame([[square feet, bedrooms, bathrooms, age]], 
    ...
    print(f"\n=== New Prediction ===")
    print(f"House features: {sqft:.0f} feet, {bedrooms} bedrooms {bathrooms} bathrooms {age} years old,")
